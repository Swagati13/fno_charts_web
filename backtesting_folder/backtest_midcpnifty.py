...
def get_strategy_stats(selected_dte,start_time, end_time, strike_distance, sl_percent, max_loss_points, target_points, get_trades = False):
    ...
    for i in range(2000):
        ...
        if date.isoweekday() == 1 and date <= dt_now:
    ...
    bnf_data_path = os.path.join(data_path, "midcpnifty", day_wise_pkl_path)
    bnf_spot_path = os.path.join(data_path, "midcpnifty", "midcpnifty_spot.pkl")
    bnf_folder = os.path.join(data_path, "midcpnifty")
    
    expiry_dates = list(set(expiry_dates))
    expiry_dates = sorted(expiry_dates, reverse=True)
    ...
    def get_midcpnifty_atm_strike(ltp):
        diff = ltp % 25
        if diff > 12.5:
            atm_strike = ltp - diff + 25
        else:
            atm_strike = ltp - diff
        return atm_strike
    ...
    for date in trading_dates:
        try:
            ...
            atm_strike = get_midcpnifty_atm_strike(ltp)
    ...
    trades_df['pnl'] = trades_df['pnl'] * 75
