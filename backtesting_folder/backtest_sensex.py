...
def get_strategy_stats(selected_dte,start_time, end_time, strike_distance, sl_percent, max_loss_points, target_points, get_trades = False):
    ...
    prev_date = ""
    for i in range(2000):
        date = datetime.datetime(2023,9,1) + datetime.timedelta(days=i)
        date_str = str(date.date())
        if date.isoweekday() == 5 and date <= dt_now:
    ...
    bnf_data_path = os.path.join(data_path, "sensex", day_wise_pkl_path)
    bnf_spot_path = os.path.join(data_path, "sensex", "sensex_spot.pkl")
    bnf_folder = os.path.join(data_path, "sensex")
