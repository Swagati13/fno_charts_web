...
def get_strategy_stats(selected_dte,start_time, end_time, strike_distance, sl_percent, max_loss_points, target_points, get_trades = False):
    ...
    for i in range(2000):
        ...
        if date.isoweekday() == 4 and date <= dt_now:
    ...
    bnf_data_path = os.path.join(data_path, "bnf", day_wise_pkl_path)
    bnf_spot_path = os.path.join(data_path, "bnf", "bnf_spot.pkl")
    bnf_folder = os.path.join(data_path, "bnf")
    
    expiry_dates = list(set(expiry_dates))
    bn_expiry_dates_df = pd.read_csv(os.path.join(bnf_folder, 'bn_expiry_dates.csv'))
    bn_expiry_dates_df["date"] = bn_expiry_dates_df["date"].apply(lambda x: datetime.datetime.strptime(x, "%Y-%m-%d"))
    bn_expiry_dates = bn_expiry_dates_df["date"].to_list()

    expiry_dates = list(set(bn_expiry_dates))
    ...
    for date in trading_dates:
        try:
            ...
            atm_strike = get_banknifty_atm_strike(ltp)
    ...
    trades_df['pnl'] = trades_df['pnl'] * 15
